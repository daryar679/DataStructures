...
/*MyVector MyVector:: sortedSquares(MyVector& vec, SortedStrategy strategy) //не работает, но пусть будет
{
	MyVector newVec(vec);
	...
	int tail = (int)vec._data[end];
	int head = (int)vec._data[begin];

	int endSquare = tail * tail;
	int beginSquare = head * head; 

	if (strategy == SortedStrategy::Decrease)
	{
		for (size_t i = 0; i < newVec._size; i++)
		{
			if (endSquare > beginSquare)
			{
				newVec._data[i] = endSquare;
				end--;
				endSquare = vec._data[end] * vec._data[end];
			}
			else
			{
				newVec._data[i] = beginSquare;
				begin++;
				beginSquare = vec._data[begin] * vec._data[begin];
			}
		}
	}
	...
	if (strategy == SortedStrategy::Increase)
	{
		for (size_t i = vec._size-1; i >= 0; i--)
		{
			...
			else
			{
				newVec._data[i] = beginSquare;
				++begin;
				beginSquare = std::pow(vec._data[begin], 2);
			}
		}
	}
	return newVec;
